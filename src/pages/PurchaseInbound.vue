    ...
    <a-card class="card-section">
      <a-form layout="inline" class="filter-bar">
        <a-form-item label="采购单号">
          <a-input v-model:value="filters.keyword" placeholder="采购单/供货商" allow-clear />
        </a-form-item>
      ...
      <a-table :columns="columns" :data-source="filteredRows" :pagination="{ pageSize: 8 }" :scroll="{ x: 1200 }">
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'status'">
            <a-tag :color="record.status === '已入库' ? 'green' : 'orange'">{{ record.status }}</a-tag>
          </template>
          <template v-else-if="column.key === 'action'">
            <a-space>
              <a-button type="link">查看</a-button>
              <a-button type="link">入库</a-button>
            </a-space>
          </template>
        </template>
      </a-table>
...
const rows = [
  { key: 'p1', id: 'PO-202602-001', supplier: '济南新大友蔬菜批发有限公司', amount: '¥12,400', status: '已入库', date: '2026-02-03' },
  { key: 'p2', id: 'PO-202602-002', supplier: '济南三胖子牛羊肉有限公司', amount: '¥8,200', status: '待入库', date: '2026-02-04' },
]

const columns = [
  { title: '采购单号', dataIndex: 'id', key: 'id', width: 160 },
  { title: '供货商', dataIndex: 'supplier', key: 'supplier', width: 220 },
  { title: '采购金额', dataIndex: 'amount', key: 'amount', width: 120 },
  { title: '状态', dataIndex: 'status', key: 'status', width: 120 },
  { title: '入库日期', dataIndex: 'date', key: 'date', width: 140 },
